...
for (const stage of document.querySelectorAll('[data-spline]')) {
  ...
  let app, loadingPromise, inView = false, failed = false;
  ...
  const observer = new IntersectionObserver(entries => {
    ...
    if (inView) { if (!manual && !failed) load(); if (!document.hidden) app?.play(); }
    else app?.stop();
  }, {rootMargin:'120px'});
  observer.observe(stage);
  document.addEventListener('visibilitychange', () => {
    if (document.hidden || !inView) app?.stop(); else app?.play();
  });
  ...
  window.addEventListener('pagehide', () => { observer.disconnect(); app?.dispose(); window.removeEventListener('message', onMessage); }, {once:true});
}
